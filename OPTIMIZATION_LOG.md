...
Entry point: `run_voxel_stream.sh` -> `voxel_stream/main.cpp`

1) Pi sender
   - H.264 RTP/UDP stream from `/dev/videoX` (`run_pi_stream_h264.sh`).
2) WSL receiver
   - GStreamer decode to BGR (`common/stream_receiver.cpp`).
3) Undistort
   - `BuildUndistortMaps` + `cv::remap`.
4) Depth
   - Depth Anything V3 TensorRT (`common/depth_estimator.hpp`).
5) Tracking
   - ORB-SLAM3 RGB-D tracking (`System::TrackRGBD`), viewer disabled.
6) Voxel integration
   - Ray-march depth into a fixed-size local grid (`VoxelMap`) in
     `voxel_stream/voxel_map.cpp` (default 5m x 5m x 3m at 0.1m voxels).
7) Snapshot for UI
...
Command:
`bash run_voxel_stream.sh --port 5600 --log-timing`
